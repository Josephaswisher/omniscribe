...
import { 
  X, Plus, FileText, Briefcase, Lightbulb, PenTool, Users, 
  Sparkles, Copy, Trash2, ChevronRight, Eye
} from 'lucide-react';
...
const templatePresets = [
  {
    name: 'Meeting Notes',
    description: 'Extract key points, decisions, and action items from meetings',
    systemPrompt: 'You are a professional meeting scribe. Analyze the transcript and extract:\n\n## Key Discussion Points\n- List the main topics discussed\n\n## Decisions Made\n- List any decisions that were agreed upon\n\n## Action Items\n- List tasks with assignees if mentioned\n\n## Follow-ups\n- List items that need follow-up',
    icon: 'users',
  ...
  },
  {
    ...
    systemPrompt: 'You are a project management assistant. Structure the update:\n\n## Progress Summary\nBrief overview of current status\n\n## Completed This Week\nList accomplishments\n\n## In Progress\nList ongoing work\n\n## Blockers/Risks\nHighlight any issues\n\n## Next Steps\nOutline upcoming priorities',
    icon: 'briefcase',
  ...
  const TemplateCard: React.FC<{ template: Template }> = ({ template }) => {
    ...
    return (
      <motion.div
        ...
        className="bg-slate-800/40 border border-white/5 rounded-xl p-4 hover:bg-slate-800/60 transition-all group"
      >
        <div className="flex items-start justify-between mb-3">
          ...
          <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button
              ...
              className="p-1.5 rounded-lg bg-slate-700/50 text-slate-400 hover:text-white"
            >
              <PenTool className="w-3.5 h-3.5" />
            </button>
            <button
              onClick={() => onUseAsParser(template)}
              className="p-1.5 rounded-lg bg-indigo-500/20 text-indigo-400 hover:bg-indigo-500/30"
            >
        ...
        <div className="flex items-center justify-between">
          <span className="text-[10px] uppercase tracking-wider text-slate-500 bg-slate-700/50 px-2 py-0.5 rounded">
            {template.category}
          </span>
    ...
    <div className="fixed inset-0 z-50 flex flex-col bg-slate-900">
            ...
            {templatePresets.map((preset, i) => {
              ...
              return (
                <motion.button
                  ...
                  className="bg-slate-800/30 border border-slate-700/50 rounded-xl p-4 text-left hover:border-indigo-500/30 transition-all"
                >
                  <div className="w-8 h-8 rounded-lg bg-indigo-500/10 flex items-center justify-center mb-3">
                    <Icon className="w-4 h-4 text-indigo-400" />
                  </div>
                  <h3 className="text-slate-200 font-medium text-sm mb-1">{preset.name}</h3>
        ...
        <section>
          ...
          {templates.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-sm text-slate-500 mt-1">Create one from scratch or use a preset</p>
            </div>
          ) : (
      ...
      <AnimatePresence>
        {showEditor && (
          ...
          >
            <header className="safe-top px-4 py-4 border-b border-slate-800 flex items-center justify-between">
              <button
              ...
              <button
                ...
                className={`p-2 rounded-lg ${previewMode ? 'bg-indigo-500 text-white' : 'bg-slate-800 text-slate-400'}`}
              >
            ...
            <div className="flex-1 overflow-y-auto safe-bottom p-4 space-y-6">
              ...
              <div>
                ...
                <div className="flex gap-2 flex-wrap">
                  {(['productivity', 'creative', 'professional', 'custom'] as const).map(cat => (
                    <button
                      key={cat}
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={10}
                  className="w-full px-4 py-3 bg-slate-800 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 resize-none font-mono text-sm"
                />
              </div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button
                  ...
                  className="flex-1 py-3 bg-indigo-500 text-white rounded-xl font-medium disabled:opacity-50 hover:bg-indigo-400"
                >
