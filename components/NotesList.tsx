...
const NotesList: React.FC<NotesListProps> = ({ notes, parsers, onNoteSelect, onDelete, onRetry, isSearching }) => {
  ...
  if (notes.length === 0) {
    if (isSearching) {
      return (
        <div className="flex flex-col items-center justify-center h-full text-slate-500/50 pb-20 pt-10">
          <svg className="w-12 h-12 mb-4 stroke-1 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
          </svg>
          <p className="text-base font-medium text-slate-400">No matches found</p>
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full text-slate-500/50 pb-20">
        <svg className="w-16 h-16 mb-6 stroke-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
  ...
  return (
    <div className="px-4 pb-32 space-y-3">
      <div className="flex items-center justify-between px-2 pt-4 pb-2">
        ...
        <span className="text-[10px] bg-slate-800 text-slate-400 px-2 py-0.5 rounded-full">{notes.length}</span>
      </div>
        ...
        >
          ...
          <div className="flex justify-between items-start mb-2">
             <div className="flex flex-col">
                <span className="text-slate-100 font-semibold text-base leading-tight">
                    {note.transcript ? (
                        note.transcript.length > 60 ? note.transcript.substring(0, 60) + '...' : note.transcript
                    ) : (
                        <span className="text-slate-500 italic">
                            {note.status === 'processing' ? 'Processing audio...' : 'Untitled Recording'}
                        </span>
                    )}
                </span>
             </div>
          ...
          <div className="flex items-center justify-between mt-3">
            ...
            <div className="flex items-center gap-3">
                ...
                <span className="text-xs font-mono text-slate-500">{formatDuration(note.duration)}</span>
            </div>
          </div>
        </div>
      ))}
