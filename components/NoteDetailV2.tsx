...
} from "./NoteVersionHistory";
import { format } from "date-fns";
import { applyTemplateToTranscript } from "../geminiService";
...
const NoteDetailV2: React.FC<NoteDetailV2Props> = ({
  ...
  // Helper to save version before making changes
  const saveVersionBeforeChange = (
    changeType: NoteVersion["changeType"],
    description?: string,
  ): NoteVersion[] => {
    const version = createVersionSnapshot(note, changeType, description);
    const existingVersions = note.versions || [];
    // Keep last 20 versions
    return [...existingVersions, version].slice(-20);
  };

  // Handle restoring a previous version
  const handleRestoreVersion = (version: NoteVersion) => {
    // First, save current state as a version
    const versionsWithCurrent = saveVersionBeforeChange(
      "manual",
      "Before restore",
    );
    // Then restore the selected version's content
    onUpdate({
      transcript: version.transcript,
      parsedSummary: version.parsedSummary,
      templateOutputs: version.templateOutputs,
      title: version.title,
  ...
  const handleApplyTemplate = async (template: Template) => {
    if (!note.transcript || isGeneratingTemplate) return;

    setIsGeneratingTemplate(true);
    try {
      // Save version before applying template
      const versions = saveVersionBeforeChange(
        "template",
        `Applied ${template.name}`,
      );

      const output = await applyTemplateToTranscript(
        note.transcript,
        template.systemPrompt,
      );
      const newOutput: TemplateOutput = {
        templateId: template.id,
        templateName: template.name,
        output,
        generatedAt: Date.now(),
      };
      const existingOutputs = note.templateOutputs || [];
      const filteredOutputs = existingOutputs.filter(
        (o) => o.templateId !== template.id,
      );
      onUpdate({
        templateOutputs: [...filteredOutputs, newOutput],
        versions,
      });
    } catch (error) {
      console.error("Failed to apply template:", error);
      alert("Failed to generate template output. Please try again.");
    } finally {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        case " ": // Space - Play/Pause
          ...
          togglePlayback();
          break;
        case "ArrowLeft": // Left arrow - Seek back 5 seconds
          e.preventDefault();
        ...
        case "ArrowRight": // Right arrow - Seek forward 5 seconds
          ...
          break;
        case "ArrowUp": // Up arrow - Seek back 10 seconds
          e.preventDefault();
          audio.currentTime = Math.max(0, audio.currentTime - 10);
          break;
        case "ArrowDown": // Down arrow - Seek forward 10 seconds
          e.preventDefault();
  ...
  const saveTitle = () => {
    if (titleDraft !== note.title) {
      const versions = saveVersionBeforeChange("title", `Changed title`);
      onUpdate({ title: titleDraft, versions });
    }
  ...
  return (
    ...
    >
      ...
      {/* Header */}
      <header className="safe-top px-4 py-4 border-b border-terminal-border/50 bg-terminal-bg/80 backdrop-blur-xl">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="w-10 h-10 rounded-full bg-terminal-surface flex items-center justify-center"
            >
              <MoreVertical className="w-5 h-5 text-neutral-200" />
            </button>
          </div>
        </div>
        ...
        <AnimatePresence>
          {showMenu && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -10 }}
              className="absolute right-4 top-20 bg-terminal-surface border border-terminal-border rounded-xl overflow-hidden shadow-xl z-10"
            >
              <button
                onClick={() => {
                ...
                className="w-full px-4 py-3 flex items-center gap-3 hover:bg-terminal-hover text-left"
              >
                <FolderOpen className="w-4 h-4 text-neutral-400" />
                <span className="text-neutral-100">Move to folder</span>
              </button>
              <button
                onClick={() => {
              ...
              >
                ...
                <span className="text-neutral-100">Add tags</span>
              </button>
              <button
                onClick={handleCopy}
                className="w-full px-4 py-3 flex items-center gap-3 hover:bg-terminal-hover text-left"
              >
                ...
                <span className="text-neutral-100">Copy text</span>
              </button>
              <button
                onClick={() => {
              ...
              >
                ...
                <span className="text-neutral-100">Version history</span>
              ...
              <button
                ...
                className="w-full px-4 py-3 flex items-center gap-3 hover:bg-terminal-hover text-left text-red-400"
              >
      ...
      <main className="flex-1 overflow-y-auto safe-bottom">
        ...
        <div className="px-6 py-4">
          {isEditingTitle ? (
            ...
            >
              ...
              <Edit3 className="w-4 h-4 text-neutral-500 opacity-0 group-hover:opacity-100 transition-opacity" />
            </button>
          )}

          <div className="flex items-center gap-3 mt-2 text-sm text-neutral-500">
            <span>{format(note.createdAt, "MMM d, yyyy • h:mm a")}</span>
            <span className="bg-terminal-surface px-2 py-0.5 rounded text-xs uppercase">
              {parser?.name || "Raw"}
            </span>
            {note.languageCode && (
              <LanguageBadge
        ...
        {/* Audio Player */}
        <div className="mx-6 bg-terminal-surface border border-terminal-border rounded-2xl p-4 mb-6">
          <div className="flex items-center gap-4 mb-4">
            <button
              onClick={togglePlayback}
              className="w-14 h-14 rounded-full bg-rgb-cyan flex items-center justify-center shadow-lg shadow-indigo-500/20"
            >
              {isPlaying ? (
              ...
              <div className="space-y-2">
                {note.bookmarks.map((bookmark) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between py-2 px-3 bg-terminal-hover/30 rounded-lg hover:bg-terminal-hover"
                  >
                    <div className="flex items-center gap-2">
        ...
        <div className="mx-6 mb-8">
          ...
          {note.status === "processing" ? (
            ...
            <TemplateOutputTabs
              ...
              summary={note.parsedSummary}
              templateOutputs={note.templateOutputs}
              templates={templates}
              isGenerating={isGeneratingTemplate}
              onApplyTemplate={handleApplyTemplate}
              onCreateTemplate={onOpenTemplateBuilder}
        ...
        {showTagPicker && (
          ...
          >
            <motion.div
              ...
              className="w-full bg-terminal-surface rounded-t-3xl p-6 safe-bottom"
            >
              ...
              <div className="space-y-2">
                <button
                  ...
                  className={`w-full flex items-center gap-3 p-3 rounded-xl transition-colors ${
                    !note.folderId
                      ? "bg-rgb-cyan/20 text-indigo-400"
                      : "bg-terminal-hover text-neutral-200 hover:bg-terminal-hover"
                  }`}
                >
